...
//	return false;
        ...
        if ($.isArray(r))
          if (r.length == 3) {
            if (r[1] in StoreProducts.product[product_id]['variations']) {
              selected_users = r[1];
              $('.nousers-' + selected_users).trigger('click');
            }
            if (r[2] in StoreProducts.product[product_id]['variations'][selected_users]) {
              selected_years = r[2];
              $('#year' + selected_years).trigger('click');
              $('#year' + selected_years + '_btm').trigger('click');
              $('#topTimeSelector-' + selected_years).trigger('click');
            }
          }
      }
    }
  ...
  if (buy_class != null) {
    // DEX-11415
    $("." + buy_class).on('click', function(e){
      buy_link = $(this).attr('href');
...
StoreProducts.__onChangeUsers = function(ev)
{
  ...
  var old_selected_years = document.querySelector(`.${c_config['years_class']}`)
...
StoreProducts.setInfo = function(buy_class, product_id, variation){
  if(buy_class != null && buy_class != ""){
    $("."+buy_class).attr('data-product', product_id);
    if(typeof(variation.discount) !== 'undefined'){
      $("."+buy_class).attr('data-buy-price',variation.discount.discounted_price);
    } else {
      $("."+buy_class).attr('data-buy-price',variation.price);
    }
    $("."+buy_class).attr('data-old-price',variation.price);
    $("."+buy_class).attr('data-currency',variation.currency_label);
    $("."+buy_class).attr('data-region',variation.region_id);
    $("."+buy_class).attr('data-variation',variation.variation.variation_name);
  }
};
...
StoreProducts.requestPricingInfo = function (so) {
  ...
  if (config.method && config.method.toLowerCase() == 'get') {
    ...
    $.get(urlGet.toString(), handleResponse);
  } else {
    $.post(url, {data: so}, handleResponse, "json");
  }
};
...
StoreProducts.filterRequestObject = function (so) {
  ...
  $.ajax({
    type: "POST",
    url: "https://onza5mmwed.execute-api.us-east-1.amazonaws.com/dev/dyn-price",
    data: requestObj,
    dataType: "json",
    timeout: 1000, // in milliseconds
    success: function(data) {
      // we need to modify so accordingly : so.config.extra_params
      if (data.discount) {
    ...
    error: function(request, status, err) {
      if (request.status == '403')
        ...
        digitalData.user.profileConsumer.dynamic = 'error|' + request.status;
